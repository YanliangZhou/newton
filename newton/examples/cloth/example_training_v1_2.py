...
###########################################################################
# Training scenario v1.2 (multi-world cloth + manipulator batch)
#
...
# batched Cartesian control. File name ``example_training_v1_2.py`` (CLI short name
# ``training_v1_2``): Python module names cannot contain ``.``, so ``v1.2`` is encoded as
# ``v1_2`` in the filename.
#
# Main features:
...
#   - Batched end-effector targets; Jacobian reuse across worlds (see Notes)
#
# Notes:
...
#   - Jacobian reuse (see ``compute_body_jacobian_world0`` + ``generate_control_joint_qd``):
#     the manipulator Jacobian ``J`` is computed only for world 0's end-effector and the same
#     ``J^+`` is applied to every world's Cartesian error. That is only correct if every world
#     uses the *same* kinematic chain and *similar* joint positions (``J`` depends on ``q``).
#     If different worlds drift to different ``q``, control quality degrades; if you load
#     *different* arm URDFs per world, this shortcut is wrong and you need a per-world ``J``.
...
# :class:`TrainingDemoV1_2Api` for a stable import surface when vendoring).
# Human-oriented usage (CLI, custom URDF / animation pools): ``TRAINING_V1_2_README.md``
# in this directory.
...
#     That folder always contains ``recording.usd`` plus ``example_training_v1_2_meta.json``
#     (fixed names); per-world JSONs are ``example_training_v1_2_meta_world_0000.json``, …
...
# ---------------------------------------------------------------------------
#
# Stable imports for downstream packages: see ``__all__`` and :class:`TrainingDemoV1_2Api`.
...
class DemoRecordingPaths:
    ...
    metadata_json_path: str = "example_training_v1_2_meta.json"
...
DEMO_SESSION_DIR_METADATA_BASE = "example_training_v1_2_meta.json"
...
def apply_demo_recording_session_paths(args: argparse.Namespace) -> None:
    ...
    ``<session>/example_training_v1_2_meta.json`` with ``session = resolve_demo_recording_session_dir(root)``,
...
def add_demo_recording_args(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "recording.usd, example_training_v1_2_meta.json, and per-world "
            "example_training_v1_2_meta_world_WWWW.json next to it (auto layout; safe for many "
...
class Example:
    def __init__(self, viewer, args, *, demo_config: DemoScenarioConfig | None = None):
        ...
        self.cloth_initial_offset_cm: tuple[float, float, float] = (10.0, 0.0, 0.0)

        # Key poses command ``body_q[ee] * endeffector_offset`` (+22 cm in tool frame), not mesh
        # finger tips; Jacobian velocity control leaves residual error. Table top z=20 cm.
        # Negative values pull low targets down. Only keyframes with z < 27 (table band).
        self.robot_ee_low_approach_z_adjust_cm = -3
        ...
        if isinstance(self.viewer, ViewerUSD) and not self.demo_config.write_usd:
            print(
                "[training_v1_2] ViewerUSD with write_usd=False: only begin/end_frame per tick "
    ...
    def _demo_print_scene_complexity(self) -> None:
        ...
        line = "[training_v1_2] scene: " + " | ".join(parts)
    ...
    def generate_control_joint_qd(self, state_in: State):
        ...
        z_adj = float(self.robot_ee_low_approach_z_adjust_cm)
        for env_id in range(self.num_envs):
            ox, oy, _ = self.world_offsets[env_id]
            self.target_xforms_np[env_id, 0] = target_local[0] + ox
            self.target_xforms_np[env_id, 1] = target_local[1] + oy
            tz = float(target_local[2])
            if tz < 27.0:
                tz += z_adj
            self.target_xforms_np[env_id, 2] = tz
    ...
    def _demo_tick_print_fps(self) -> None:
        ...
        if elapsed >= 1.0:
            ...
            print(
                f"[training_v1_2] loop FPS (GPU-sync wall): {fps:.2f}  "
    ...
    def build_demo_metadata_world_dict(self, world_index: int, *, metadata_json_path: Path | None = None) -> dict:
        ...
        robot_block: dict = {
            "control_scheme": "task_space_keyframes_jacobian_world0_reused",
        ...
        return {
            "schema": "newton.examples.training_v1_2/world/v3",
    ...
    def build_demo_metadata_dict(self) -> dict:
        ...
        return {
            "schema": "newton.examples.training_v1_2/aggregate/v2",
            "world_count": self.num_envs,
            "usd_path": p.usd_path,
...
def write_demo_metadata(example: Example, paths: DemoRecordingPaths | None = None) -> list[Path]:
    ...
    if not example.demo_config.write_metadata_json:
        print("[training_v1_2] skipped metadata JSON (write_metadata_json=False)", flush=True)
        return []
...
def run_training_v1_2_demo(
    *,
    ...
    if session_note:
        print(f"[training_v1_2] recording session dir: {session_note}", flush=True)
...
class TrainingDemoV1_2Api:
    ...
    See ``TRAINING_V1_2_README.md`` (same directory) for CLI examples, programmatic
    ...
    notes at the end of ``TRAINING_V1_2_README.md``.
    ...
    run_full_demo = staticmethod(run_training_v1_2_demo)
...
__all__ = [
    ...
    "make_demo_scenario",
    "run_training_v1_2_demo",
    "write_demo_metadata",
]


if __name__ == "__main__":
    run_training_v1_2_demo(parser_defaults={"num_frames": 3850})
