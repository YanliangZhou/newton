...
###########################################################################
# 25x Cloth Franka Batch
#
# Based on example_cloth_franka.py, but creates 25 independent copies
# of the Franka + table + cloth scene in a 5x5 grid and runs them together.
#
# Main changes:
#   - 25 Franka articulations, one per world
#   - 25 tables
#   - 25 procedurally generated panels (different shape / size / stiffness)
...
class Example:
    def __init__(self, viewer, args):
        ...
        print("bodies_per_world =", self.bodies_per_world)
        print("expected bodies =", self.num_envs * self.bodies_per_world)
        ...
        # Increase camera distance to see all 25 robots
        self.viewer.set_camera(wp.vec3(center_x, center_y, 10.0), -90.0, 0.0)  # Top-down view
        ...
        print(f"Number of worlds: {self.num_envs}")
        ...
        # Disable automatic world offsets since we already positioned the robots manually
        # Set world offsets to zero to prevent double positioning
        self.viewer.set_world_offsets((0.0, 0.0, 0.0))
        
        # Ensure all worlds are visible by explicitly listing them
        visible_worlds = list(range(self.num_envs))
        self.viewer.set_visible_worlds(visible_worlds)
        print(f"Set visible worlds: {visible_worlds}")
        
        # Print viewer information
        print(f"Viewer model set with all worlds visible")
        if hasattr(self.viewer, "world_offsets"):
            print(f"Viewer world offsets: {self.viewer.world_offsets}")
        if hasattr(self.viewer, "_shape_instances"):
            print(f"Number of shape instances: {len(self.viewer._shape_instances)}")
...
def test_25x_arms():
    """Test function to verify 25 Franka arms are properly created and positioned."""
    print("Testing 25x Franka arms...")
    ...
    for i in range(num_envs):
        ...
        if i == 0:
            ...
            print(f"Each arm has {bodies_per_world} bodies")
    ...
    print("=== Body Positions (first 10 bodies) ===")
