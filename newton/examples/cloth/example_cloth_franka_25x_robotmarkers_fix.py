...
@wp.kernel
def extract_body_markers(src: wp.array[wp.transform], scale: float, dst: wp.array[wp.transform]):
    i = wp.tid()
    p = wp.transform_get_translation(src[i])
    dst[i] = wp.transform(p * scale, wp.quat_identity())
...
class Example:
    def __init__(self, viewer, args):
        ...
        # Manual robot debug visualization: the default articulated visual mesh path
        # can fail to show all robot instances in large batched layouts, so we also
        # draw a small marker at every robot body origin.
        self.robot_marker_body_ids = np.arange(self.model.body_count, dtype=np.int32)
        self.robot_marker_scale = (0.035, 0.035, 0.035)
        self.robot_marker_color = wp.array([
            wp.vec3(0.95, 0.25, 0.25) if (i % self.bodies_per_world) < (self.bodies_per_world - 2)
            else wp.vec3(0.25, 0.8, 0.95)
        ...
        self.robot_marker_xforms = wp.empty(self.model.body_count, dtype=wp.transform)
    ...
    def render(self):
        ...
        if self.model.body_count > 0:
            ...
            wp.launch(
                ...
                dim=self.model.body_count,
                inputs=[self.state_0.body_q, self.viz_scale],
                outputs=[self.robot_marker_xforms],
            )
        ...
        self.viewer.log_shapes("/robot_markers", newton.GeoType.SPHERE, self.robot_marker_scale, self.robot_marker_xforms, self.robot_marker_color)
