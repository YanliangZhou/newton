...
#   - Dynamic trajectories and meshes go to USD (``--output-path`` with
#     ``--viewer usd``); per-world sidecar JSON files are written next to the path
#     given by ``--demo-metadata-json`` (see :func:`write_demo_metadata`).
#
...
class Example:
    ...
    def _demo_tick_print_fps(self) -> None:
        """Once per main-loop iteration: print rolling FPS to the terminal every 1 s wall clock."""
